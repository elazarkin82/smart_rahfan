...
def build_reference_template(image, center, crop_size, target_size):
    """
    Builds a single target reference template.
    Extracts a square crop of 'crop_size' around the center, pads if needed,
    resizes to 'target_size'x'target_size', and expands channel dimension.
    """
    crop = get_crop(image, center[0], center[1], crop_size)
...
def main():
    ...
    tgt_sz = compiler_cfg.get('template_target_size', 128)
    ...
    for cache_path in tqdm.tqdm(cache_files, desc="Compiling Dataset"):
        ...
        ref_stack = build_reference_template(
            frame_0['image_gray'], 
            frame_0['target_2d'], 
            flight_max_sz, tgt_sz
        )
        ...
        for k, frame_dict in enumerate(flight_data[1:]):
            ...
            sample = {
                "reference_template": ref_stack,            # Shape: (128, 128, 1)
                "reference_stack": ref_stack,
