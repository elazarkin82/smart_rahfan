...
class KerasFCNVisualizer:
    def __init__(self, root, dataset_dir, model_path, threshold=0.5, min_blob_size=30):
        ...
        print(f"Loading Keras TargetTrackerVerPixel from {model_path}...")
        ...
        self.root = root
        self.root.title("TargetTrackerVerPixel - Live Inference Visualizer")
    ...
    def setup_ui(self):
        header = tk.Label(self.root, text="TargetTrackerVerPixel Visualizer", font=("Outfit", 15, "bold"), bg="#121212", fg="#ffffff")
        header.pack(pady=15)
    ...
    def load_next_sample(self):
        try:
            ...
            ref_template = sample.get("reference_template", sample.get("reference_stack"))
            ...
            # Detect reference crop type and prepare visualizer image source
            if len(ref_template.shape) == 4 and ref_template.shape[0] == 16:
                # Legacy stack (16, 32, 32, 1)
                ref_vis_source = ref_template[0, :, :, 0]
                ref_tensor = tf.expand_dims(tf.cast(ref_template, tf.float32) / 255.0, 0)
            else:
                # Single template (128, 128, 1)
                if len(ref_template.shape) == 3:
                    ref_vis_source = ref_template[:, :, 0]
                else:
                    ref_vis_source = ref_template[0, :, :, 0]
                ref_tensor = tf.expand_dims(tf.cast(ref_template, tf.float32) / 255.0, 0)
                
            ref_vis = cv2.resize(ref_vis_source, (256, 256), interpolation=cv2.INTER_LINEAR)
...
def main():
    ...
    parser.add_argument("--dataset_dir", default="dataset_generator/dataset/")
