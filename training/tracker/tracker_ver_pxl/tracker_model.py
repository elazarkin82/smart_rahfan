...
class TargetTrackerVerPixel:
    def __init__(self, ref_shape=(128, 128, 1), search_shape=(256, 256, 1), config_path="model.conf"):
    ...
    def create_model(self):
        ref_input = layers.Input(shape=self.ref_shape, name="reference_template") # (128, 128, 1)
        ...
        # 2. Process Reference Template
        ref_outputs = shared_backbone(ref_input)
        ref_features = ref_outputs[-1] # Shape: (None, 8, 8, C) since input is (128, 128, 1) and downsamples 16x
        
        # 3. Process Search Frame
        # Extract features (including skip connection features for decoder)
...
def parse_training_samples(pkl_path):
    ...
    refs = np.stack([s.get('reference_template', s.get('reference_stack')) for s in samples], axis=0)
...
def build_tf_dataset(pkl_files, shuffle=True):
    ...
    output_signature = (
        (
            tf.TensorSpec(shape=(None, 128, 128, 1), dtype=tf.uint8, name="reference_template"),
    ...
    def process_element(inputs, targets):
        ...
        return {"reference_template": ref_float, "search_frame": search_float}, {"predicted_heatmap": heatmap_resized, "predicted_quality": tf.cast(quality, tf.float32)}
