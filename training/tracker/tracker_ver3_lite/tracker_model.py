...
# =====================================================================
# Helper Functions for Exponential Cone Masking and Target Heatmaps
# =====================================================================
...
def generate_exponential_mask(coords, size=256, sigma=30.0):
    """
    Generates a 2D sharp exponential cone attention mask of shape (size, size, 1)
    centered around normalized coordinates to maximize sub-pixel gradient sensitivity.
    
    Formula: M(d) = exp(-d / sigma)
    """
    ...
    # Compute Euclidean distance from target point
    d = np.sqrt((x_grid - x_target) ** 2 + (y_grid - y_target) ** 2)
    # Apply standard exponential decay
    mask = np.exp(-d / sigma)
...
def generate_exponential_heatmap(coords, size=64, sigma=4.0):
    """
    Generates a 2D sharp exponential cone target heatmap centered around a normalized 
    coordinate to preserve continuous sub-pixel ratios and high gradient sensitivity.
    
    Formula: H(d) = exp(-d / sigma)
    """
    x_target = coords[0] * size
    y_target = coords[1] * size
...
def focal_loss(y_true, y_pred, alpha=0.25, gamma=2.0):
    """
    Stable Sigmoid Focal Loss designed for continuous target heatmaps.
    Uses safe clipping to avoid NaNs.
    """
    eps = 1e-7
...
# =====================================================================
# Target Tracker Lite Class (Symmetric 2-Frame Model)
# =====================================================================

class TargetTracker:
    """
    An ultra-lightweight, symmetric, drift-free Siamese template tracker in Keras.
    
    Key Properties:
      - Tracks using only 2 frames: 'hist_input' (Anchor Template) and 'curr_input' (Search Frame).
      - Siamese Backbone operates on a clean 1-channel grayscale image for perfect weight-sharing symmetry.
      - Inject target location from 'hist_input' after the backbone using Late Mask Fusion gating.
      - Uses Depthwise Separable Convolutions in the decoder for ultra-fast mobile inference.
      
    Inputs:
      1. hist_input: (256, 256, 2) -> [hist_frame_grayscale, hist_exponential_mask]
      2. curr_input: (256, 256, 1) -> [curr_frame_grayscale]
      
    Output:
      Regresses a continuous spatial probability distribution (Exponential Cone Heatmap) of shape (64, 64, 1).
    """
    def __init__(self, hist_shape=(256, 256, 2), curr_shape=(256, 256, 1)):
    ...
    def create_model(self):
        """
        Builds the Fully Convolutional Siamese 2-Frame Keras model.
        Returns:
            tf.keras.Model: The uncompiled Keras FCN model.
        """
        # 1. Setup inputs
        hist_input = layers.Input(shape=self.hist_shape, name="hist_input")
        curr_input = layers.Input(shape=self.curr_shape, name="curr_input")
        
        # 2. Slice hist_input into Image (Channel 0) and Mask (Channel 1)
        hist_img = layers.Lambda(lambda x: x[:, :, :, 0:1], name="slice_hist_img")(hist_input)
        hist_mask = layers.Lambda(lambda x: x[:, :, :, 1:2], name="slice_hist_mask")(hist_input)
        
        # 3. Instantiate Shared 1-Channel Siamese Backbone
        cnn_backbone = self._create_cnn_backbone()
        
        # 4. Extract visual features symmetrically at (16, 16, 128)
        hist_features = cnn_backbone(hist_img)
        curr_features = cnn_backbone(curr_input)
        
        # 5. Late Mask Fusion Gating
        # Downsample hist_mask from (256, 256, 1) to (16, 16, 1) using Average Pooling
        hist_mask_downsampled = layers.AveragePooling2D(pool_size=16, name="downsample_mask")(hist_mask)
        # Gate historical features: zero-out/weight visual representations outside the mask
        hist_features_gated = layers.Multiply(name="late_mask_gating")([hist_features, hist_mask_downsampled])
        
        # 6. Feature Interaction Mapping
        # Compute element-wise correlation/interaction map (template matching)
        interaction_features = layers.Multiply(name="feature_interaction")([curr_features, hist_features_gated])
        
        # 7. Concatenate visual representations: (16, 16, 384)
        fused_features = layers.Concatenate(axis=-1, name="siamese_fusion")(
        ...
        self.model = models.Model(
            inputs=[hist_input, curr_input],
            outputs=output_heatmap,
            name="TargetTrackerLite"
        )
    ...
    def load_dataset_from_pickles(dataset_dir, eval_pkl_num=4):
        ...
        Expects pickles to contain 2-frame inputs:
          - inputs[0] (hist): shape (None, 256, 256, 2)
          - inputs[1] (curr): shape (None, 256, 256, 1)
        """
        ...
        if not pickle_files:
            raise FileNotFoundError(f"No .pkl or .pickle files found in '{dataset_dir}'.")
        ...
        output_signature = (
            (
                ...
                tf.TensorSpec(shape=(None, 256, 256, 1), dtype=tf.float32, name="curr_input"),
            ),
    ...
    def train(self, train_dataset, val_dataset, lr, num_of_epochs, loss_name="mse", output_path=None, best_train_loss_output=None, log_file=None):
        ...
        for epoch in range(1, num_of_epochs + 1):
            ...
            if best_train_loss_output is not None and epoch_loss < best_train_loss:
                ...
                # Save structural model with clean weights
                temp_tracker = TargetTracker(hist_shape=self.hist_shape, curr_shape=self.curr_shape)
...
def main(args_list=None):
    ...
    parser = argparse.ArgumentParser(description="TargetTracker Lite (FCN Symmetric 2-Frame) CLI Tool")
    subparsers = parser.add_subparsers(dest="command", required=True, help="Available subcommands")
    
    # Subparser for train
    train_parser = subparsers.add_parser("train", help="Train the TargetTracker Lite model with symmetric inputs")
    train_parser.add_argument(
        "--dataset_dir", 
        required=True, 
        help="Directory containing the .pkl 2-frame dataset batches"
    )
    train_parser.add_argument(
        "--lr", 
        type=float, 
    ...
    train_parser.add_argument(
        ...
        type=int, 
        default=4, 
        help="Number of initial pickle files for evaluation (default: 4)"
    )
    train_parser.add_argument(
        "--init_keras_file", 
        type=str, 
        default=None, 
        help="Path to initial Keras model to resume training from"
    )
    train_parser.add_argument(
        "--output", 
        type=str, 
        default=None, 
        help="Path to save the best validation model"
    )
    train_parser.add_argument(
        "--best_train_loss_output", 
        type=str, 
        default=None, 
        help="Path to save the best training loss model"
    )
    train_parser.add_argument(
        "--log_file", 
        type=str, 
